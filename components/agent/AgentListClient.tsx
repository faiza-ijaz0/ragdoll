...
async function fetchAgentProperties(agentId: string): Promise<Property[]> {
  try {
    ...
    console.log(`Total unique properties for agent ${agentId}: ${uniqueProperties.length}`)
    return uniqueProperties.slice(0, 3) // Sirf 3 properties show karein card mein
  ...
  try {
    ...
    const agentsWithProperties = await Promise.all(
      agents.map(async (agent) => {
        try {
          const properties = await fetchAgentProperties(agent.id)
          return {
            ...agent,
            properties
          }
        } catch (error) {
          console.error(`Error fetching properties for agent ${agent.id}:`, error)
          return agent
        }
      })
    )
    ...
    agentsWithProperties.sort((a, b) => (b.experience_years || 0) - (a.experience_years || 0))
...
export default function AgentListClient() {
  ...
  // Open agent properties modal
  const openAgentProperties = (agent: Agent) => {
    setSelectedAgentForProperties(agent)
    setShowPropertiesModal(true)
    document.body.style.overflow = 'hidden'
  }

  // Close agent properties modal
  const closeAgentProperties = () => {
    setShowPropertiesModal(false)
    setSelectedAgentForProperties(null)
    document.body.style.overflow = 'auto'
  }
  ...
  return (
    <>
                ...
                {['all', 'Luxury', 'Commercial', 'Residential', 'Investment', 'New Developments'].map(
                  (spec) => (
                    <button
                      ...
                      className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 ${filter === spec
      ...
      {showPropertiesModal && selectedAgentForProperties && (
        <AgentProperties
          agentId={selectedAgentForProperties.id}
          agentName={selectedAgentForProperties.title}
