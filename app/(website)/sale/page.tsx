                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-white border border-slate-200 rounded-xl p-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(agentData?.specializations || ['Sales Properties']).map((spec: string, idx: number) => (
                        <span 
                          key={idx} 
      ...
      <div className="fixed inset-0 z-50 bg-white overflow-y-auto">
        ...
        <div className="container-custom pt-8 pb-12">
          <div className="flex flex-row gap-3 mb-6">
            ...
            <p className="text-xl mb-2 font-bold text-slate-500">Properties For Sale</p>
          </div>
                  ...
                  <div className="absolute top-4 left-4 right-4 flex justify-between">
                    <div className="flex gap-2">
                      <span className="px-4 py-2 bg-primary text-white text-sm font-bold rounded-full shadow-lg">
                        {property.status === "sale" ? "FOR SALE" : "PROPERTY"}
                      </span>
                      {property.featured && (
                  ...
                  <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div className="space-y-4">
                      ...
                      <h1 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tight leading-tight">
                        {property.title || "Property for Sale in Dubai"}
                      </h1>
                    ...
                    <div className="prose prose-slate max-w-none">
                      <p className="text-slate-600 leading-relaxed text-lg whitespace-pre-line font-medium bg-slate-50 p-8 rounded-3xl border border-slate-100">
                        {property.description ||
                          "Experience luxury living in this property located in Dubai. This residence offers modern amenities and prime location."}
                      </p>
                    </div>
                  </div>
...
async function fetchSalePropertiesFromMainCollection() {
  try {
    ...
    console.error('Error fetching SALE from main collection:', error.message);
    return [];
  }
}
...
async function fetchSalePropertiesFromAgentCollection() {
  try {
    ...
    console.error('Error fetching SALE from agent collection:', error.message);
    return [];
  }
}
...
async function fetchAllSaleProperties() {
  try {
    const [mainProperties, agentProperties] = await Promise.all([
      ...
      fetchSalePropertiesFromAgentCollection()
    ]);
...
function PropertiesPageContent() {
  ...
  const action = searchParams.get('action') || 'sale';
  ...
  useEffect(() => {
    ...
    setFormState({
      action: searchParams.get('action') || 'sale',
