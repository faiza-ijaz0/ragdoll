...
// Types define karen - Updated with proper video type
interface Project {
  id: string;
  name: string;
  developer?: string;
...
// Request Information Form Component
...
const FullScreenGallery = ({
  ...
  // Get all media (images + videos) for the project - FIXED: Use videos array
  const getProjectMedia = () => {
    ...
    // Add ALL videos from videos array - FIXED
    ...
    // Also check for old video_url field for backward compatibility
    else if (project.video_url && getVideoType(project.video_url) !== "none") {
      media.push({
        type: "video",
...
function ProjectsPageContent() {
  ...
  // Get all media (images + videos) for a project - FIXED: Use videos array
  const getProjectMedia = (project: Project) => {
    ...
    if (project.videos && project.videos.length > 0) {
      project.videos.forEach((video) => {
        if (getVideoType(video.url) !== "none") {
          media.push({
            type: "video",
            url: video.url,
            thumbnail: video.url,
  ...
  const renderCurrentMedia = (project: Project) => {
    ...
    const currentMedia = projectMedia[currentVideoIndex];
  ...
  const handleDownload = (url: string, fileName: string) => {
    // Agar Base64 hai to direct download
    if (url.startsWith('data:application/pdf') || url.startsWith('data:image')) {
      ...
      // Agar URL hai to new tab mein open karo
  ...
  const getDocumentIcon = (type?: string) => {
    ...
    if (type.includes('pdf')) return <DocumentIcon className="h-6 w-6 text-red-500" />;
    if (type.includes('image')) return <PhotoIcon className="h-6 w-6 text-green-500" />;
    if (type.includes('video')) return <VideoCameraIcon className="h-6 w-6 text-blue-500" />;
          ...
          <div className="container-custom pt-8 pb-12 mt-25">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
              ...
              <div className="lg:col-span-8 space-y-12">
                {/* Media Gallery (Images + Multiple Videos) - FIXED */}
                <div className="overflow-hidden shadow-2xl rounded-3xl shadow-slate-200/50">
                        ...
                        if (projectMedia.length > 0) {
                          ...
                          return (
                            <div className="absolute top-2 right-4 px-3 py-1 bg-black/50 text-white text-sm rounded-full flex items-center gap-1 z-20">
                              ...
                              {currentVideoIndex + 1} / {projectMedia.length}
                            </div>
                          );
                        }
                        return null;
                  ...
                  {/* Thumbnail Gallery - FIXED to show all videos */}
                  {(() => {
                    const projectMedia = getProjectMedia(detailsModal.project);
                    if (projectMedia.length > 1) {
                      return (
 ...
 {/* Brochures Section */}
                    {detailsModal.project.brochures && detailsModal.project.brochures.length > 0 && (
                      <div className="space-y-6 pt-10 border-t border-slate-100">
                        <h2 className="text-2xl font-black text-slate-900 flex items-center gap-3">
                          <span className="w-8 h-8 bg-primary/10 rounded-lg flex items-center justify-center text-primary">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {detailsModal.project.brochures.map((brochure, index) => (
                            <div
                              key={index}
                              className="bg-slate-50 rounded-2xl border border-slate-200 p-5 hover:shadow-lg transition-all duration-300 group"
                            >
                              <div className="flex items-start gap-4">
                                {/* Icon */}
                                <div className="shrink-0 w-12 h-12 bg-red-100 rounded-xl flex items-center justify-center">
                                  {brochure.type === 'base64' ? (
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <p className="text-xs text-slate-500 mt-1">
                                    {brochure.type === 'base64' ? 'PDF Document' : 'Online PDF'}
                                  </p>
                                </div>

                                {/* Download Button */}
                                <button
                                  onClick={() => handleDownload(brochure.url, brochure.name || `brochure-${index + 1}.pdf`)}
                                  className="shrink-0 w-10 h-10 bg-white rounded-full flex items-center justify-center text-red-600 hover:bg-red-600 hover:text-white transition-colors shadow-sm"
                                  title="Download PDF"
                                >
                    ...
                    {detailsModal.project.other_documents && detailsModal.project.other_documents.length > 0 && (
                      <div className="space-y-6 pt-10 border-t border-slate-100">
                        <h2 className="text-2xl font-black text-slate-900 flex items-center gap-3">
                          <span className="w-8 h-8 bg-primary/10 rounded-lg flex items-center justify-center text-primary">
                            <DocumentIcon className="w-5 h-5" />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {detailsModal.project.other_documents.map((doc, index) => (
                            <div
                              key={index}
                              className="bg-slate-50 rounded-2xl border border-slate-200 p-5 hover:shadow-lg transition-all duration-300 group"
                            >
                              <div className="flex items-start gap-4">
                                ...
                                {/* Download Button */}
                                <button
                                  onClick={() => handleDownload(doc.url, doc.name || `document-${index + 1}.pdf`)}
                                  className="shrink-0 w-10 h-10 bg-white rounded-full flex items-center justify-center text-orange-600 hover:bg-orange-600 hover:text-white transition-colors shadow-sm"
