...
//   // Get brochures directly from property
//   const brochures: {name: string, url: string, directUrl: string}[] = [];
...
//   const [selectedBrochure, setSelectedBrochure] = useState(brochures[0] || null);
...
//   // Get floorplans and documents directly from property
//   const floorplans: {name: string, url: string, directUrl: string}[] = [];
//   const otherDocuments: {name: string, url: string, directUrl: string}[] = [];
...
//   const [selectedDocument, setSelectedDocument] = useState(otherDocuments[0] || null);
//   const [activeTab, setActiveTab] = useState<'floorplans' | 'documents'>(floorplans.length > 0 ? 'floorplans' : 'documents');
...
//   // Get videos directly from property
...
//   const renderVideo = (video: {url: string, type: string}) => {
//     if (videoError) {
//       return (
...
//   // Get all media (images and videos combined) directly from property
...
//             type: 'video',
//             url: videoUrl,
//             thumbnail: getVideoThumbnail(videoUrl, videoType, index + mediaItems.length)
//           });
//         }
//       }
//     });
...
//           type: 'video',
//           url: videoUrl,
//           thumbnail: getVideoThumbnail(videoUrl, videoType, mediaItems.length)
//         });
//       }
//     }
//   }
...
//                     {agentData?.title || "Commercial Real Estate Agent"}
//                   </h2>
//                   <p className="text-white/80 text-lg">{agentData?.brokerage || "Commercial Property Specialist"}</p>
//                 </div>
...
//                       {(agentData?.specializations || ['Commercial', 'Office', 'Retail', 'Industrial']).map((spec: string, idx: number) => (
//                         <span 
//                           key={idx} 
...
//                     </h3>
//                     <div className="flex flex-wrap gap-2">
//                       {(agentData?.areas || ['Dubai', 'Abu Dhabi', 'Sharjah']).map((area: string, idx: number) => (
//                         <span 
//                           key={idx} 
...
//                     </h3>
//                     <div className="space-y-3">
//                       {(agentData?.certifications || ['RERA Certified', 'Commercial Specialist']).map((cert: string, idx: number) => (
...
//   // Combine images and videos directly from property
//   const mediaItems: {type: 'image' | 'video', url: string, thumbnail?: string}[] = [];
...
//     });
//   } else if (property.image) {
//     mediaItems.push({ type: 'image', url: property.image, thumbnail: property.image });
//   }
...
//   // Set agent data directly from property
//   useEffect(() => {
...
//                           {property.type?.toUpperCase() || "COMMERCIAL PROPERTY"}
//                         </span>
...
//                         {property.title || "Commercial Property in Dubai"}
//                       </h1>
...
//                           "Prime commercial property located in Dubai. This space offers excellent business potential and strategic location."}
//                       </p>
//                     </div>
//                   </div>
...
//                       Get detailed information about this commercial property.
//                     </p>
...
//                               Complete commercial property information
//                             </p>
//                           </div>
//                         </div>
//                       </button>
...
//                           className="group p-6 bg-slate-50 hover:bg-red-600 hover:text-white border border-slate-200 hover:border-red-600 rounded-2xl transition-all duration-300 text-left"
//                         >
//                           <div className="flex items-center gap-4">
//                             <div className="w-12 h-12 bg-red-100 group-hover:bg-white/20 rounded-xl flex items-center justify-center">
//                               <VideoCameraIcon className="w-6 h-6 text-red-600 group-hover:text-white" />
//                             </div>
//                             <div>
//                               <h3 className="text-lg font-black text-slate-900 group-hover:text-white">
//                                 Video Tour
//                               </h3>
//                               <p className="text-sm text-slate-600 group-hover:text-white/80">
//                                 Watch commercial property videos
//                               </p>
//                             </div>
//                           </div>
//                         </button>
...
//           type: detailedProperty.type || detailedProperty.subtype || 'Commercial',
...
//           baths: p.baths ?? 0,
//           sqft: p.sqft ?? 0,
//           type: p.type || p.subtype || 'Commercial',
...
//     router.replace(`/commercial`, { scroll: false });
//   };
...
//     if (showOnlyVideos) {
//       desc = 'Watch video tours of prime commercial ';
//     } else {
//       desc = 'Discover prime commercial ';
//     }
...
//     desc += '. Browse our curated selection of premium business spaces with detailed information and high-quality images.';
...
//               )}
//               {showOnlyVideos && (
//                 <span className="px-6 py-2 bg-red-500/20 backdrop-blur-md text-red-500 rounded-full border border-red-500/30 text-sm font-bold">
//                   📹 Video Tours
//                 </span>
//               )}
//             </div>
...
//                     : showOnlyVideos 
//                       ? 'No commercial properties with video tours found matching your criteria.'
...
//   return (
//     <Suspense fallback={<div className="min-h-screen bg-slate-50/50 flex items-center justify-center"></div>}>
...
// ===== VIDEO HELPER FUNCTIONS - DEFINED BEFORE USE =====
const getVideoType = (url: string) => {
  if (!url || typeof url !== 'string') return 'none';
  if (url.includes('youtube.com') || url.includes('youtu.be') || url.includes('youtube.com/embed')) return 'youtube';
  if (url.includes('vimeo.com')) return 'vimeo';
...
function FloorPlanForm({
  ...
  if (property.documents && Array.isArray(property.documents)) {
   ...
   // FIXED: Allow null in state types
  const [selectedFloorplan, setSelectedFloorplan] = useState<{name: string, url: string, directUrl: string} | null>(floorplans[0] || null);
  const [selectedDocument, setSelectedDocument] = useState<{name: string, url: string, directUrl: string} | null>(otherDocuments[0] || null);
  const [activeTab, setActiveTab] = useState<'floorplans' | 'documents'>(floorplans.length > 0 ? 'floorplans' : 'documents');
              ...
              <div className="overflow-hidden shadow-2xl rounded-3xl shadow-slate-200/50">
                <div className="relative h-[500px] bg-slate-100 overflow-hidden">
                  ...
                  <button
                    ...
                    className="absolute bottom-6 right-6 bg-black/70 hover:bg-black/90 text-white px-5 py-3 rounded-xl flex items-center gap-2 transition-colors backdrop-blur-sm shadow-lg z-10"
                  >
                    <ArrowsPointingOutIcon className="h-5 h-5" />
                    <span className="font-bold text-sm">Open Gallery</span>
                  </button>
